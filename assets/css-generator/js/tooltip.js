...
var Tooltip = (function($, window, undefined) {
  ...
  var $tooltip = false;//$('<div><span class="mm-tooltip-cf7" onclick="return false;">Title</span><div class="hidden"><p><b>HTML</b> for your tooltip!</p></div></div>').appendTo(".preview");

  // Css classes for the styled element
  var css_classes = {
    base: 'cf-chat-response',
    text: 'cf-chat-response text',
    userBase: 'cf-chat-response.user',
    userText: 'cf-chat-response.user text',
    robotAvatar: 'cf-chat-response.robot thumb',
    userAvatar: 'cf-chat-response.user thumb'
  };
  ...
  var set = {
    robotAvatar: function(value) {
      css_definitions.robotAvatar['background-image'] = value;
    },
    userAvatar: function(value) {
      css_definitions.userAvatar['background-image'] = value;
    },
    fontColor: function(value) {
      css_definitions.base.color = value;
    },
    userFontColor: function(value) {
      css_definitions.userBase.color = value;
    },
    ...
    backgroundColor: function(baseColor) {
      css_definitions.text['background-color'] = baseColor;
    ...
    userBackgroundColor: function(baseColor) {
      css_definitions.userText['background-color'] = baseColor;
    },
    ...
    userBorderColor: function(value) {
      css_definitions.userBase['border-color'] = value;
    },

    borderWidth: function(value) {
      css_definitions.base['border-width'] = value;
    },
    ...
    borderRadius: function(value) {
      ...
      $.extend(css_definitions.userBase, {
        '-webkit-border-radius': px_value,
        '-moz-border-radius': px_value,
        'border-radius': px_value
      });
  ...
  $.extend(this, {
    ...
    maxBorderRadius: function() {
      return 30;//parseInt($tooltip.outerHeight() / 2) + 30;
    },
    ...
    paint: function() {
      ...
      $.each(self.styles, function(name, style) {
        console.log('setstyle', name, style);
  ...
  if(typeof(parent.window) != 'undefined') {
    //console.log(parent.window.jQuery('#mcfgf-conversation-generator-css-options').val());
    eval('var savedStyles = ' + parent.window.jQuery('#mcfgf-conversation-generator-css-options').val());
    eval('var savedOptions = ' + parent.window.jQuery('#mcfgf-conversation-generator-js-code').val());
